...
const Header = () => {
    ...
    return (
        <header className="fixed w-full z-10 flex justify-center top-2">
            <nav
                className="border border-neutral-600/60 bg-background-light/70 backdrop-blur-sm rounded-4xl p-2 flex items-center justify-between w-full max-w-[800px]">
                <div>
                ...
                <div className="flex gap-4 items-center">
                    <Link className="bg-white rounded-full p-0.5 opacity-75 hover:opacity-100 transition-opacity"
                        aria-label={t("npmLink")}
                        href={"https://www.npmjs.com/package/@toryx/core"}
                        target="_blank">
                        <Image className="rounded-full" src={"/icons/npm.svg"} alt="npm" width={30} height={30} />
                    </Link>
                    <Link className="bg-white rounded-full p-0.5 opacity-75 hover:opacity-100 transition-opacity"
                        aria-label={t("gitHubLink")}
                        href={"https://github.com/scalprox/toryx"}
                        target="_blank">
